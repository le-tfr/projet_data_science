...
# Importation et concaténation de MOD1
mod1_parts = []
for i in range(1, 9):
...
# Importation et concaténation de MOD2
mod2_parts = []
for i in range(1, 9):
...
# Importation et concaténation de PODs
pod_files = {
...
# Convertion de la colonne 'Time' au format datetime avec la timezone
...
# Convertion des colonnes Started et Ended en format datetime avec le bon fuseau horaire
...
# Initialisation des variables pour stocker les instances d'activités et la longueur totale
activity_instances = {}
activity_lengths = {}
...
def normalize_instance(instance, common_length):
    ...
    f = interp1d(np.arange(original_length), instance_array, kind='linear', axis=0, bounds_error=False, fill_value="extrapolate")
...
# Création d'une liste de toutes les instances d'activité et une liste des longueurs moyennes
...
# Calcule des signatures moyennes pour toutes les activités
avg_activity_signatures = averageSignatures(all_instances, all_lengths)

# Impression les signatures moyennes pour chaque activité
...
# Concaténation de tous les sous-ensembles d'activités en un seul dataframe
...
input_size = len(activity_df.columns) - 2 
num_classes = 10
...
def create_sequences(dataset, seq_length):
    ...
    for i in range(len(dataset) - seq_length):
        ...
        seqs.append(seq)
        label = dataset.iloc[i+seq_length-1, -1]
        labels.append(label)
    if seqs and labels:
...
# Détermination de la longueur des séquences pour créer les données d'entrée du LSTM
seq_length = 20

# Création des séquences pour chaque activité
activity_sequences = {}
for activity, instances in activity_instances.items():
    ...
    if sequences_and_labels:
        activity_sequences[activity] = sequences_and_labels

# Concaténation de toutes les séquences et les étiquettes pour les différentes activités
...
hidden_size = 64
num_layers = 2
num_classes = len(activity_label_mapping)
